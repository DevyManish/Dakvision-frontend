...
export default function EmployeeDashboard() {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get("http://localhost:8080/api/v1/employee/view/743135")
  ...
  const handleAddEmployee = async (event) => {
    ...
    try {
      await axios.post("http://localhost:8080/api/v1/employee/create", newEmployee)
  ...
  const handleEditEmployee = async (event) => {
    ...
    try {
      await axios.put("http://localhost:8080/api/v1/employee/edit", updatedEmployee)
      fetchEmployees() // Refresh the employee list
  ...
  const handleDeleteEmployee = async (empId) => {
    // Note: There's no delete API provided, so we'll just remove it from the local state
    ...
    <div className="flex flex-col mt-16 gap-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{Math.round(employees.length * 0.8)}</div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <form onSubmit={handleEditEmployee} className="space-y-4">
                          ...
                          <div className="grid w-full gap-2">
                            <Label htmlFor="edit-performance">Performance Score (0-100)</Label>
                            <Input 
                              id="edit-performance" 
                              name="performance" 
                              type="number" 
